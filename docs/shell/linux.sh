...
#####################################################################
#            FILE: linux.sh
#           USAGE: bash linux.sh [artifact name] [project root path] [store artifact dir] [store generate scripts dir]
#
#     DESCRIPTION:
#               build linux binary artifact for x64 platform
#         OPTIONS:
#
#    REQUIREMENTS:
#
...
if [[ $# -ne 4 ]]; then
    show_color_text_by_echo "Usage: bash linux.sh [artifact name] [project root path] [store artifact dir] [store generate scripts dir]"
    exit 1
fi
...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -o "${STORE_ARTIFACT_DIR}/bin/${ARTIFACT_TARGET}"

if [[ $? -eq 0 ]] && \
    [[ -f ${STORE_ARTIFACT_DIR}/bin/${ARTIFACT_TARGET} ]]; then
    chmod +x ${STORE_ARTIFACT_DIR}/bin/${ARTIFACT_TARGET}

    show_color_text_by_echo "[*] build ${ARTIFACT_TARGET} on linux for x86 platform success" "green"
    ...
    show_color_text_by_echo "[*] archive ${ARTIFACT_TARGET} to ${STORE_ARTIFACT_DIR}.tar.gz success" "green"
     ...
     show_color_text_by_echo "[*] build ${ARTIFACT_TARGET} on linux for x86 platform fail" "red"
     exit 1
fi
